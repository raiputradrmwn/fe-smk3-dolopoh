// src/app/profil/waka-kurikulum/components/WakaKurikulumHero.tsx
"use client";
...
export default function WakaKurikulumHero() {
  const waka = React.useMemo(
    () => WAKA.find((p) => p.id === "waka-kurikulum"),
    []
  );
  if (!waka) return null;
  ...
  return (
    <section className="text-center">
      ...
      <p className="mt-2 text-gray-600">{waka.role}</p>

      <div className="mt-8 flex justify-center">
        <PortraitPhoto
          src={waka.photo || "/images/placeholder-portrait.jpg"}
          alt={`Foto ${waka.name}`}
          size="xl"
          priority
      ...
      <Card className="mx-auto mt-8 max-w-lg border-gray-200 shadow-sm">
        <CardContent className="flex justify-center gap-4 p-6">
          <InfoBox
            icon={MapPin}
            label="Tempat, Tgl Lahir"
            value={`${waka.birthPlace ?? "—"}, ${waka.birthDate ?? "—"}`}
          />
        </CardContent>
      </Card>
