// src/app/profil/waka-kesiswaan/components/WakaKesiswaanHero.tsx
"use client";
...
export default function WakaKesiswaanHero() {
  const waka = React.useMemo(
    () => WAKA.find((p) => p.id === "waka-kesiswaan"),
    []
  );
  if (!waka) return null;
      ...
      <Card className="mx-auto mt-8 max-w-3xl border-gray-200 shadow-sm">
        <CardContent className="grid grid-cols-1 gap-4 p-6 sm:grid-cols-3">
          ...
          <InfoBox
            icon={GraduationCap}
            label="Pendidikan"
            value={waka.education ?? "—"}
          />
          <InfoBox
            icon={CalendarDays}
            label="Bekerja Sejak"
            value={`${waka.startedAt ?? "—"} – sekarang`}
          />
        </CardContent>
      </Card>
